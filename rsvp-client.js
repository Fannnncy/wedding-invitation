...
const rsvpListStorageKey = 'wedding-local-rsvps'
let savedRsvp
...
rsvpForm.addEventListener('submit', async (event) => {
  ...
  rsvpSubmit.textContent = rsvpConfig.mode === 'local' ? '正在保存到本机……' : '正在送往鹈鹕镇……'
  try {
    if (rsvpConfig.mode === 'local') {
      const now = new Date().toISOString()
      const id = submission.id || window.crypto?.randomUUID?.() || `${Date.now()}-${Math.random()}`
      savedRsvp = { ...submission, id, submitted: true, createdAt: savedRsvp?.createdAt || now, updatedAt: now }
      const records = JSON.parse(window.localStorage.getItem(rsvpListStorageKey) || '[]')
      const nextRecords = Array.isArray(records) ? records.filter((item) => item.id !== id) : []
      nextRecords.unshift(savedRsvp)
      window.localStorage.setItem(rsvpListStorageKey, JSON.stringify(nextRecords))
    } else {
    ...
    try { window.localStorage.setItem(rsvpStorageKey, JSON.stringify(savedRsvp)) } catch { /* Storage may be unavailable. */ }
